...
let Random = (function () {
    ...
    function nextCircleVector(rotation) {
        rotation = rotation + Math.PI / 2;
        if (rotation + Math.PI / 4 > 2 * Math.PI) {
            rotation -= 2 * Math.PI;
        }
        let angle = Math.random() * 2 * Math.PI;
        while (angle < rotation - Math.PI / 4 || angle > rotation + Math.PI / 4) {
            if (angle < rotation - Math.PI / 4) {
                angle += Math.PI / 4;
            } else if (angle > rotation + Math.PI / 4) {
                angle -= Math.PI / 4;
            }
        }
        return {
